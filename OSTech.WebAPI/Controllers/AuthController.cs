    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] LoginModel model)
        {
            ...
            if (user is not null && await _userManager.CheckPasswordAsync(user, model.Password!))
            {
                ...
                var refreshToken = _tokenService.GenerateRefrestToken();

                _ = int.TryParse(_configuration["JWT:RefreshTokenValidityInMinutes"],
                                             out int refreshTokenValidityInMinutes);

                user.RefreshTokenExpiryToken =
        ...
        public async Task<ActionResult> RefreshToken(TokenModel tokenModel)
        {
            ...
            if (user == null || user.RefreshToken != refreshToken
                            || user.RefreshTokenExpiryToken <= DateTime.UtcNow)
                return BadRequest("Invalid access token/refresh token");

            var newAccessToken = _tokenService.GenerateAccessToken(
                                               principal.Claims.ToList(), _configuration);

            var newRefreshToken = _tokenService.GenerateRefrestToken();
